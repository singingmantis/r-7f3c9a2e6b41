...
class PlayerSubtitlesTest {
    @Test fun acceptsObservedSrcFieldAndPrefersFullSubtitles() {
        val config = jacksonObjectMapper().readTree("""{"subs":[{"label":"Türkçe (Forced)","src":"/forced.vtt"},{"label":"Tam (Türkçe)","src":"/full.vtt"},{"label":"Broken","src":"http://[bad"}]}""")
        val subs = playerSubtitles(config, "https://player.example/watch")
        assertEquals(listOf("Tam (Türkçe)" to "https://player.example/full.vtt", "Türkçe (Forced)" to "https://player.example/forced.vtt"), subs)
    }
